...
namespace HdmiExtenderLib
{
    public class HdmiExtenderReceiver
    {
        ...
        public HdmiExtenderReceiver(string ipAddressOfSenderDevice, string staticNetworkAdapterIP)
        {
            this.addressSenderDevice = IPAddress.Parse(ipAddressOfSenderDevice);
            this.networkAdapterIP = staticNetworkAdapterIP;
        }
        ...
        private void doDataStream()
        {
            try
            {
                ...
                {
                    if (LivePacketDevice.AllLocalMachine[i].Addresses[1].Address.ToString().Contains(networkAdapterIP))
                    {
                        selectedDevice = LivePacketDevice.AllLocalMachine[i];
                    }
                ...
                {
                    abort = true;
                    Console.WriteLine("Can't find a networkadapter that contains IP: " + networkAdapterIP);
                    return;
                }
