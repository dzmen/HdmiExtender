...
namespace HdmiExtenderLib
{
    public class VideoWebServer : HttpServer
    {
        ...
        public VideoWebServer(int port, int port_https, string senderIPAddress, string staticNetworkAdapterIP)
            : base(port, port_https)
        {
            receiver = new HdmiExtenderReceiver(senderIPAddress, staticNetworkAdapterIP);
            receiver.Start();
        }
